...
/*   Updated: 2025/07/31 13:08:29 by ko-mahon         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	load_images(t_map *map)
{
	...
	map->img_player = mlx_xpm_file_to_image(map->mlx, "sprite/ryuk.xpm", &w, &h);
	map->img_collectible = mlx_xpm_file_to_image(map->mlx,
			"sprite/pomme.xpm", &w, &h);
...
int	main(int argc, char **argv)
{
	...
	map.img_w = 64;
	map.img_h = 64;
	if (init_and_validate_map(&map, argv[1]))
		return (1);
	map.mlx = mlx_init();
	if (!map.mlx)
		return (clean_exit(&map, "MLX init failed\n", 1));
	map.win = mlx_new_window(map.mlx, map.width * map.img_w,
			map.height * map.img_h, "so_long");
	if (!map.win)
